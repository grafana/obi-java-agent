...
public class IOCTLPacket {
    ...
    public static int writePacketPrefix(Pointer mem, int off, OperationType type, Socket socket, int bufLen) {
    ...
    public static int writePacketPrefix(Pointer mem, int off, OperationType type, Connection conn, int bufLen) {
        mem.setByte(off, type.code);
        off ++;
        if (conn == null) {
            off = ConnectionInfo.writeEmptyConnectionInfo(mem, off);
